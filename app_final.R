    ...
    data.frame(
        ...
        x %>% group_by(Date) %>%
            summarise(
                ...
                Delta_Value = sum(Delta_Value, na.rm = T)
            ),
        iso3 = "NULL"
    )
}
...
modify.data <- function(x) {
    ...
    x[x$iso3 %in% list, "Country.Region"] <-
        x[x$iso3 %in% list, "Province.State"]
    ...
    function(x,
        ...
        hcmap(
            ...
            hc_colorAxis(
                ...
                minColor = minColor,
                maxColor =  maxColor,
                allowNegativeLog = allowNegativeLog
            ) %>%
...
regression <- function(x, country_list) {
    do.call("rbind", sapply(country_list, function(c) {
        ...
        colnames(d)[2:3] <-
            c("log10 Growth Factor", "Exponential Growth Factor")
        d
    }, simplify = F))
}
...
active <- data.frame(
    Country.Region = confirmed$Country.Region,
    Date = confirmed$Date,
...
all <-
    cbind(confirmed[which(!colnames(confirmed) == "iso3")], deaths[which(!colnames(deaths) == "iso3")],
...
all$Fnl_Death_pct <-
    all[["Deaths(#)"]] / rowSums(all[c("Deaths(#)", "Recovered(#)")], na.rm = T) *
    100
...
all$Active_pct <-
    100 - all[["Overall Death Rate(%)"]] - all[["Overall Recovery Rate(%)"]]
...
dyn_data <- function() {
    a <-
        jsonlite::fromJSON("https://api.covid19india.org/data.json",
                           simplifyVector = TRUE)
    india <-
        a$cases_time_series[c(
            "dailyconfirmed",
            "dailydeceased",
            "dailyrecovered",
            "date",
    ...
    india[which(!colnames(india) == "date")] <-
        apply(india[which(!colnames(india) == "date")], 2, as.numeric)
    colnames(india) <-
        c(
            "New Confirmed(#)",
            "New Deaths(#)",
    ...
    india$Active <-
        india[["Confirmed(#)"]] - india[["Deaths(#)"]] - india[["Recovered(#)"]]
    colnames(india)[length(colnames(india))] <- "Active(#)"
    
    india$Death_pct <-
        india[["Deaths(#)"]] / india[["Confirmed(#)"]] * 100
    colnames(india)[length(colnames(india))] <-
        "Overall Death Rate(%)"
    india$Fnl_Death_pct <-
        india[["Deaths(#)"]] / rowSums(india[c("Deaths(#)", "Recovered(#)")], na.rm = T) *
        100
    colnames(india)[length(colnames(india))] <-
        "Finalised Death Rate(%)"
    india$Recovery_pct <-
        india[["Recovered(#)"]] / india[["Confirmed(#)"]] * 100
    colnames(india)[length(colnames(india))] <-
        "Overall Recovery Rate(%)"
    india$Fnl_Recovery_pct <-
        100 - india[["Finalised Death Rate(%)"]]
    colnames(india)[length(colnames(india))] <-
        "Finalised Recovery Rate(%)"
    india$Active_pct <-
        100 - india[["Overall Death Rate(%)"]] - india[["Overall Recovery Rate(%)"]]
    colnames(india)[length(colnames(india))] <- "Active Rate(%)"
    
    india <- india[order(india$Date), ]
    
    india$Growth_Factor <-
        india[["New Confirmed(#)"]] / lag(india[["New Confirmed(#)"]])
    ...
    u <-
        jsonlite::fromJSON("https://api.covid19india.org/states_daily.json",
                           simplifyVector = T)
    ...
    colnames(u1) <-
        c("date",
          ...
          "growth_factor")
    u1$state <-
        tolower(trimws(u1$state , whitespace = "[ \t\n\r\v\f]"))
    u1 <- u1[order(u1$state), ]
    
    
    india_statewise <-
        data.frame(a$statewise[c(
            "active",
            "confirmed",
            "deaths",
            "lastupdatedtime",
    ...
    india_statewise$state1 <-
        tolower(trimws(
            gsub(" ", "", india_statewise$statecode, fixed = TRUE),
            whitespace = "[ \t\n\r\v\f]"
        ))
    india_statewise <-
        india_statewise[order(india_statewise$state1), ]
    
    india_statewise[which(
        !colnames(india_statewise) %in% c("lastupdatedtime", "state", "state1", "statecode")
    )] <-
        apply(india_statewise[which(
            !colnames(india_statewise) %in% c("lastupdatedtime", "state", "state1", "statecode")
        )], 2, as.numeric)
    ...
    india_statewise$Active <-
        india_statewise[["Confirmed(#)"]] - india_statewise[["Deaths(#)"]] - india_statewise[["Recovered(#)"]]
    colnames(india_statewise)[length(colnames(india_statewise))] <-
        "Active(#)"
    
    india_statewise$Death_pct <-
        india_statewise[["Deaths(#)"]] / india_statewise[["Confirmed(#)"]] * 100
    colnames(india_statewise)[length(colnames(india_statewise))] <-
        "Overall Death Rate(%)"
    india_statewise$Fnl_Death_pct <-
        india_statewise[["Deaths(#)"]] / rowSums(india_statewise[c("Deaths(#)", "Recovered(#)")], na.rm = T) *
        100
    colnames(india_statewise)[length(colnames(india_statewise))] <-
        "Finalised Death Rate(%)"
    india_statewise$Recovery_pct <-
        india_statewise[["Recovered(#)"]] / india_statewise[["Confirmed(#)"]] *
        100
    colnames(india_statewise)[length(colnames(india_statewise))] <-
        "Overall Recovery Rate(%)"
    india_statewise$Fnl_Recovery_pct <-
        100 - india_statewise[["Finalised Death Rate(%)"]]
    colnames(india_statewise)[length(colnames(india_statewise))] <-
        "Finalised Recovery Rate(%)"
    india_statewise$Active_pct <-
        100 - india_statewise[["Overall Death Rate(%)"]] - india_statewise[["Overall Recovery Rate(%)"]]
    colnames(india_statewise)[length(colnames(india_statewise))] <-
        "Active Rate(%)"
    
    india_statewise$Date <-
        as.Date(india_statewise$Date, format = '%d/%m/%Y')
    india_statewise$Growth_Factor <- u1$growth_factor
    colnames(india_statewise)[length(colnames(india_statewise))] <-
        "Growth Factor"
    
    india_statewise <-
        india_statewise[order(india_statewise$`Confirmed(#)`, decreasing = T), ]
    ...
    india_statewise <-
        india_statewise[, c(1, 5, 6, 2, 15, 7, 10:11, 3, 8, 12:13, 4, 9, 14)]
    india_statewise[india_statewise$Country.Region == "Total", "Country.Region"] <-
        "India"
    ...
    #india raw data
    d <-
        jsonlite::fromJSON("https://api.covid19india.org/raw_data.json",
                           simplifyVector = T)
    india_raw_data <-
        d$raw_data[c(
            "agebracket",
    ...
    india_raw_data$agebracket <-
        cut(
            as.numeric(india_raw_data$agebracket),
            breaks  = c(5, 10, 15, 20, 30, 40, 50, 60, 70 , 80),
            right = F,
            ordered_result = T
        )
    ...
    #district data
    b <-
        jsonlite::fromJSON("https://api.covid19india.org/state_district_wise.json",
                           simplifyVector = T)
    ...
    district_data <- do.call('rbind', sapply(names(b), function(x) {
        do.call('rbind', sapply(names(b[[x]][["districtData"]]), function(y) {
            data.frame(
                State = x,
                District = y,
                Confirmed = b[[x]][["districtData"]][[y]][["confirmed"]],
    ...
    district_data[c("State", "District")] <-
        apply(district_data[c("State", "District")], 2, as.character)
    district_data[district_data$District == "Unknown", "District"] <-
        "Unknown District"
    
    district_data[district_data$State == district_data$District, "District"] <-
        paste(district_data[district_data$State == district_data$District, "State"], "District")
    ...
    u1[c("confirmed", "death", "recovered")] <-
        apply(u1[c("confirmed", "death", "recovered")], 2, f_na)
    ...
    state_daily <-
        merge.data.frame(u1, statecode, by.x = "state", by.y = "statecode")
    ...
    state_daily$Active <-
        state_daily[["Confirmed(#)"]] - state_daily[["Deaths(#)"]] - state_daily[["Recovered(#)"]]
    colnames(state_daily)[length(colnames(state_daily))] <-
        "Active(#)"
    
    state_daily$Death_pct <-
        state_daily[["Deaths(#)"]] / state_daily[["Confirmed(#)"]] * 100
    colnames(state_daily)[length(colnames(state_daily))] <-
        "Overall Death Rate(%)"
    state_daily$Fnl_Death_pct <-
        state_daily[["Deaths(#)"]] / rowSums(state_daily[c("Deaths(#)", "Recovered(#)")], na.rm = T) *
        100
    colnames(state_daily)[length(colnames(state_daily))] <-
        "Finalised Death Rate(%)"
    state_daily$Recovery_pct <-
        state_daily[["Recovered(#)"]] / state_daily[["Confirmed(#)"]] * 100
    colnames(state_daily)[length(colnames(state_daily))] <-
        "Overall Recovery Rate(%)"
    state_daily$Fnl_Recovery_pct <-
        100 - state_daily[["Finalised Death Rate(%)"]]
    colnames(state_daily)[length(colnames(state_daily))] <-
        "Finalised Recovery Rate(%)"
    state_daily$Active_pct <-
        100 - state_daily[["Overall Death Rate(%)"]] - state_daily[["Overall Recovery Rate(%)"]]
    colnames(state_daily)[length(colnames(state_daily))] <-
        "Active Rate(%)"
    ...
    state_daily[state_daily$Country.Region == "Total", "Country.Region"] <-
        "India"
    ...
    state_daily <-
        state_daily[state_daily$Country.Region != "India", ]
    ...
    essentials <-
        jsonlite::fromJSON("https://api.covid19india.org/resources/resources.json",
                           simplifyVector = TRUE)
    
    return(
        list(
            india = india,
            india_raw_data = india_raw_data,
...
time_diff <- function(x, current_time = Sys.time()) {
    if (round(difftime(current_time, x, unit = "secs"), 0) <= 1) {
        paste(round(difftime(current_time, x, unit = "secs"), 0), "second ago")
    }
    else if (round(difftime(current_time, x, unit = "secs"), 0) < 60) {
        paste(round(difftime(current_time, x, unit = "secs"), 0), "seconds ago")
    }
    else if (round(difftime(current_time, x, unit = "mins"), 0) <= 1) {
        paste(round(difftime(current_time, x, unit = "mins"), 0), "minute ago")
    }
    else if (round(difftime(current_time, x, unit = "mins"), 0) < 60) {
        paste(round(difftime(current_time, x, unit = "mins"), 0), "minutes ago")
    }
    else if (round(difftime(current_time, x, unit = "hours"), 0) <= 1) {
        paste(round(difftime(current_time, x, unit = "hours"), 0), "hour ago")
    }
    else if (round(difftime(current_time, x, unit = "hours"), 0) < 24) {
        paste(round(difftime(current_time, x, unit = "hours"), 0), "hours ago")
    }
    else if (round(difftime(current_time, x, unit = "days"), 0) <= 1) {
        paste(round(difftime(current_time, x, unit = "days"), 0), "day ago")
    }
    else if (round(difftime(current_time, x, unit = "days"), 0) < 7) {
        paste(round(difftime(current_time, x, unit = "days"), 0), "days ago")
    }
    else if (round(difftime(current_time, x, unit = "weeks"), 0) < 52) {
        paste(round(difftime(current_time, x, unit = "weeks"), 0), "weeks ago")
    }
}

df_india <- get_everything(
    query = c("COVID-19 India"),
    from = Sys.Date() - 1,
    to = Sys.Date(),
    sort_by = "relevancy",
    language = "en"
...
df_australia <- get_everything(
    query = c("COVID-19 Australia"),
    from = Sys.Date() - 1,
    to = Sys.Date(),
    sort_by = "relevancy",
    language = "en"
...
map.australia <-
    "https://code.highcharts.com/mapdata/countries/au/au-all.js"
map.india <-
    "https://code.highcharts.com/mapdata/countries/in/custom/in-all-disputed.js"
...
confirmed_au <-
    modify.data.au(confirmed_0[confirmed_0$Country.Region == "Australia", ])
deaths_au <-
    modify.data.au(deaths_0[deaths_0$Country.Region == "Australia", ])
recovered_au <-
    modify.data.au(recovered_0[recovered_0$Country.Region == "Australia", ])

active_au <-
    data.frame(
        Country.Region = confirmed_au$Country.Region,
        Date = confirmed_au$Date,
        Value = confirmed_au$Value - deaths_au$Value - recovered_au$Value
    )
#Ration of first derivative
...
australia$Death_pct <-
    australia[["Deaths(#)"]] / australia[["Confirmed(#)"]] * 100
colnames(australia)[length(colnames(australia))] <-
    "Overall Death Rate(%)"
australia$Fnl_Death_pct <-
    australia[["Deaths(#)"]] / rowSums(australia[c("Deaths(#)", "Recovered(#)")], na.rm = T) *
    100
colnames(australia)[length(colnames(australia))] <-
    "Finalised Death Rate(%)"
australia$Recovery_pct <-
    australia[["Recovered(#)"]] / australia[["Confirmed(#)"]] * 100
colnames(australia)[length(colnames(australia))] <-
    "Overall Recovery Rate(%)"
australia$Fnl_Recovery_pct <-
    100 - australia[["Finalised Death Rate(%)"]]
colnames(australia)[length(colnames(australia))] <-
    "Finalised Recovery Rate(%)"
australia$Active_pct <-
    100 - australia[["Overall Death Rate(%)"]] - australia[["Overall Recovery Rate(%)"]]
...
pop <-
    merge.data.frame(pop, iso3166[c("charcode", "charcode3")], by.x = "charcode", by.y = "charcode")
pop$value <- pop$value * 1000

cnf_dth <-
    merge.data.frame(confirmed,
                     deaths,
                     by.x = c("Date", "iso3"),
                     by.y = c("Date", "iso3"))

cnf_dth_pop <-
    merge.data.frame(cnf_dth, pop, by.x = "iso3", by.y = "charcode3")

cnf_dth_pop <- cnf_dth_pop[, c(1, 3, 2, 4, 8, 11)]
colnames(cnf_dth_pop) <-
    c("iso3",
      "Country.Region",
      "Date",
      "Confirmed",
      ...
      "Population")
cnf_dth_pop$cnf_pop <-
    cnf_dth_pop$Confirmed / cnf_dth_pop$Population * (10 ^ 6)
    ...
    dashboardPagePlus(
        ...
        dashboardHeaderPlus(
            ...
            dropdownMenu(
                ...
                badgeStatus = "primary",
                icon = icon("database"),
                headerText = h4(strong("Data Sources")),
                notificationItem(
                    icon = icon("external-link"),
                    status = "primary",
                    text = "India Live Data Source",
                    href = "https://api.covid19india.org"
                ),
                notificationItem(
                    icon = icon("external-link"),
                    status = "primary",
                    text = "World Data Source",
                ...
                tabItems(
                    tabItem(
                        ...
                        box(
                            ...
                            column(
                                width = 6,
                                offset = 0.5,
                                uiOutput("analysis_type_india_select_input")
                            ),
                        ...
                        box(
                            title = "Live Cases in India Data",
                            solidHeader = T,
                            width = 12,
                            collapsible = T,
                            status = "primary",
                            popify(
                                ...
                                title = "<b>Compare Statistical Trends</b>",
                                placement = "right",
                        ...
                        box(
                            title = "Pivot Table from Raw Data",
                            solidHeader = T,
                            width = 12,
                            collapsible = T,
                            status = "primary",
                    ...
                    tabItem(
                        ...
                        box_function(
                            ...
                            "warning",
                            "plot_confirmed",
                            "growth_table_confirmed",
                            width = 12
                        ),
                        box(
                            title = "Daily Cumulative Confirmed Cases Post First 'N' Cases",
                            solidHeader = T,
                            width = 12,
                            collapsible = T,
                            status = "warning",
                        ...
                        box(
                            title = "Ratio First Derivates of Cumulative Growth",
                            solidHeader = T,
                            width = 12,
                            collapsible = T,
                            status = "primary",
                        ...
                        box(
                            ...
                            column(12, div(
                                style = 'overflow-x: scroll', DT::dataTableOutput("news_world")
                            ))
                        )
                    ),
                    tabItem("e",
                            column(12, offset = 0.5, uiOutput("source_heading"))),
                    tabItem(
                        "f",
                        column(12, offset = 0.5, h1(strong(
                            "Australia Daily Update"
                        ))),
                        column(12, offset = 0.5, htmlOutput("australia_header")),
                        htmlOutput("value_boxes4"),
                        column(12, offset = 0.5, uiOutput("axis_type_aus")),
                        box(
                            title = "Daily Cases in Australia",
                            solidHeader = T,
                            width = 12,
                            collapsible = T,
                            status = "primary",
                            ...
                            column(
                                12,
                                highchartOutput("plot_statewise_australia", height = '700px')
                            ),
                            column(
                                12,
                                offset = 0.5,
                                h3(strong("Australia Forecast")),
                                p("Forecast not available for new cases and growth factor.")
                            ),
                            column(
                                6,
                        ...
                        box(
                            title = "Daily Cases in Australia Data",
                            solidHeader = T,
                            width = 12,
                            collapsible = T,
                            status = "primary",
                    ...
                    tabItem(
                        "g",
                        column(12, offset = 0.5, h1(
                            strong(
                                "Susceptible, Infected (Active Cases) and Removed (Deaths + Recovered) Model"
                            )
                        )),
                        box(
                            title = "Toy Model",
                            ...
                            status = "primary",
                            p(
                                "This is a toy model where you change the infection and recovery rates and analyse decrease in susceptible population,
                        ...
                        box(
                            ...
                            status = "primary",
                            p(
                                "This is the most basic version SIR Model that minimises the sum of squares between
                            ...
                            fluidRow(
                                column(
                                    ...
                                    wellPanel(
                                        ...
                                        numericInput(
                                            inputId = "pop_size",
                                            label = "Susceptible Population Size",
                                            value = 10 ^ 5,
                                            min = 1,
                                            max = NA,
                                            step = 1,
                    ...
                    tabItem(
                        "h",
                        box(
                            title = "Essentials and Resources",
                            solidHeader = T,
                            width = 12,
                            collapsible = T,
                            status = "primary",
                    ...
                    tabItem(
                        ...
                        box(
                            title = "Who is Winning the War against COVID-19?",
                            solidHeader = T,
                            width = 12,
                            collapsible = T,
                            status = "primary",
                            p(
                                "'Winning the war' is a visual  analysis based on bubble chart between 'cases per million' and 'Deaths per 100 cases; size of the bubble is a measure of population.
                            ...
                            column(
                                12,
                                offset = 0.5,
                                p("*Size of the bubble represents the population size")
                            )
                        )
                    )
                )
...
server <- function(input, output, session) {
    output$map_plot_confirmed <- renderHighchart({
        ...
        x[x$Value < 1, "Value"] <- 1
        x[x$Delta_Value < 1 &
              is.na(x$Delta_Value) == FALSE, "Delta_Value"] <- 1
    ...
    filtered <- reactive({
        list(
            confirmed = confirmed[confirmed$Date >= input$daterange[1] &
                                      confirmed$Date <= input$daterange[2] &
                                      confirmed$Country.Region %in% input$countries, ],
            deaths = deaths[deaths$Date >= input$daterange[1] &
                                deaths$Date <= input$daterange[2] &
                                deaths$Country.Region %in% input$countries, ],
            recovered = recovered[recovered$Date >= input$daterange[1] &
                                      recovered$Date <= input$daterange[2] &
                                      recovered$Country.Region %in% input$countries, ],
            active = active[active$Date >= input$daterange[1] &
                                active$Date <= input$daterange[2] &
                                active$Country.Region %in% input$countries, ]
        )
    })
    ...
    output$plot_confirmed <- renderHighchart({
        ...
        type <- "linear"
        type <-
            ifelse(input[["axis_type_confirmed"]] == "Linear", "linear", "logarithmic")
        if (choice == "Cumulative") {
    ...
    output$plot_confirmed_n <- renderHighchart({
        ...
        x <- x[x$Value >= input$N, ]
        x <-
            x %>% group_by(Country.Region) %>% mutate(Days = Date - min(Date))
        type <- "linear"
        type <-
            ifelse(input[["axis_type_N"]] == "Linear", "linear", "logarithmic")
        highcharter::hchart(x, type = 'line',
                            hcaes(
                                y = Value,
            ...
            hc_yAxis(title = list(text = "Cumulative Confirmed Cases"),
                     type = type) %>%
            hc_xAxis(title = list(text = paste(
                "Days From First", input$N, "Cases"
            ))) %>%
            hc_legend(align = "right", layout = "proximate") %>%
    ...
    output$plot_deaths <- renderHighchart({
        ...
        type <- "linear"
        type <-
            ifelse(input[["axis_type_deaths"]] == "Linear", "linear", "logarithmic")
        if (choice == "Cumulative") {
            d <- x[, c("Country.Region", "Date", "Value")]
    ...
    output$plot_recovered <- renderHighchart({
        ...
        type <- "linear"
        type <-
            ifelse(input[["axis_type_recovered"]] == "Linear", "linear", "logarithmic")
        if (choice == "Cumulative") {
            d <- x[, c("Country.Region", "Date", "Value")]
    ...
    output$plot_active <- renderHighchart({
        ...
        type <- "linear"
        type <-
            ifelse(input[["axis_type_active"]] == "Linear", "linear", "logarithmic")
        d <- x[, c("Country.Region", "Date", "Value")]
    ...
    output$plot_death_rate <- renderHighchart({
        name <-
            ifelse(input$choice3 == "Overall",
                   "Overall Death Rate(%)",
                   "Finalised Death Rate(%)")
        x <-
            all[all$Date >= input$daterange[1] &
                    all$Date <= input$daterange[2] &
                    all$Country %in% input$countries, c("Country", "Date", name)]
    ...
    output$plot_proportion <- renderHighchart({
        c <- input$c
        cnf <-
            confirmed[confirmed$Country.Region == c, -match("iso3", colnames(confirmed))]
        dth <-
            deaths[deaths$Country.Region == c, -match("iso3", colnames(deaths))]
        rec <-
            recovered[recovered$Country.Region == c, -match("iso3", colnames(recovered))]
        ...
        highcharter::hchart(d, type = 'column', hcaes(
            y = value,
            group = variable,
            x = Date
        )) %>%
    ...
    output$growth_table_confirmed <- DT::renderDataTable({
        ...
        if (choice == "New") {
            ...
            x <- confirmed
            x <-
                x[x$Date >= input$daterange[1] & x$Date <= input$daterange[2], ]
            c <- input$countries
            x <-
                x[x$Country.Region %in% c, c("Country.Region", "Value")]
            d <- regression(x, c)
    ...
    output$plot_ratio_confirmed <- renderHighchart({
        ...
        x <- confirmed
        x <-
            x[x$Country.Region %in% c &
                  x$Date >= input$daterange[1] &
                  x$Date <= input$daterange[2], c("Country.Region", "Date", "Value", "Ratio_Value")]
        d <- regression(x, c)
        ...
        highchart() %>%
            hc_add_series(
                ...
                data = d,
                type = "line",
                hcaes(y = Actual, x = as.Date(Date))
            ) %>%
            hc_add_series(
                name = "Fitted",
                data = d,
                type = "line",
                hcaes(y = Fitted, x = as.Date(Date)),
    ...
    output$value_boxes <- renderUI({
        cnf <-
            comma(confirmed[confirmed$Country.Region == "World" &
                                confirmed$Date == input$date, "Value"], digits = 0)
        cnf_new <-
            comma(confirmed[confirmed$Country.Region == "World" &
                                confirmed$Date == input$date, "Delta_Value"], digits = 0)
        dth <-
            comma(deaths[deaths$Country.Region == "World" &
                             deaths$Date == input$date, "Value"], digits = 0)
        dth_new <-
            comma(deaths[deaths$Country.Region == "World" &
                             deaths$Date == input$date, "Delta_Value"], digits = 0)
        rec <-
            comma(recovered[recovered$Country.Region == "World" &
                                recovered$Date == input$date, "Value"], digits = 0)
        rec_new <-
            comma(recovered[recovered$Country.Region == "World" &
                                recovered$Date == input$date, "Delta_Value"], digits = 0)
        act <-
            comma(active[active$Country.Region == "World" &
                             active$Date == input$date, "Value"], digits = 0)
        list(
            valueBox(
                cnf,
                icon = icon("plus-square"),
                subtitle = paste0("Confirmed (+", cnf_new, ")"),
                color = "yellow",
                width = 3
            ),
            valueBox(
                ...
                subtitle = paste0(
                    ...
                    round(dth / cnf * 100, 0),
                    "% overall; ",
                    round(dth / sum(rec, dth, na.rm = T) * 100, 0),
                    "% of closed)"
                ),
                color = "red",
                width = 3
            ...
            valueBox(
                ...
                subtitle = paste0(
                    ...
                    round(rec / cnf * 100, 0),
                    "% overall; ",
                    round(100 - dth / sum(rec, dth, na.rm = T) * 100, 0),
                    "% of closed)"
                ),
                color = "green",
                width  = 3
            ...
            valueBox(
                act,
                icon = icon("diagnoses"),
                subtitle = paste0("Active", " (", round(act / cnf * 100, 0), "%)"),
                color = "aqua",
                width = 3
            )
        )
    ...
    output$value_boxes2 <- renderUI({
        cnf <-
            comma(confirmed[confirmed$Country.Region == "World" &
                                confirmed$Date == input$date2, "Value"], digits = 0)
        cnf_new <-
            comma(confirmed[confirmed$Country.Region == "World" &
                                confirmed$Date == input$date2, "Delta_Value"], digits = 0)
        dth <-
            comma(deaths[deaths$Country.Region == "World" &
                             deaths$Date == input$date2, "Value"], digits = 0)
        dth_new <-
            comma(deaths[deaths$Country.Region == "World" &
                             deaths$Date == input$date2, "Delta_Value"], digits = 0)
        rec <-
            comma(recovered[recovered$Country.Region == "World" &
                                recovered$Date == input$date2, "Value"], digits = 0)
        rec_new <-
            comma(recovered[recovered$Country.Region == "World" &
                                recovered$Date == input$date2, "Delta_Value"], digits = 0)
        act <-
            comma(active[active$Country.Region == "World" &
                             active$Date == input$date2, "Value"], digits = 0)
        list(
            valueBox(
                cnf,
                icon = icon("plus-square"),
    ...
    output$plot_modal <- renderHighchart({
        x <-
            all[all$Country %in% country(), c("Country", "Date", input$analysis_type)]
        colnames(x) <- c("Country.Region", "Date", "Value")
        type <- "linear"
        if (input$analysis_type %in% c(
            ...
            type <- ifelse(input$axis_type2 == "Log10",
                           "logarithmic",
                           "linear")
        }
        ...
        if (input$analysis_type %in% c(
            ...
            p <- p %>% hc_tooltip(valueDecimals = 2)
        }
    ...
    output$modal <- renderUI({
        bsModal(
            id = "modal_compare",
            title = strong("Comparison of Trends"),
            trigger = "compare",
            size = "large",
            column(
                width = 12,
    ...
    output$analysis_type_india_select_input <- renderUI({
        selectInput("analysis_type_india",
                    label = "Select",
                    choices = names(india())[3:15])
    })
    ...
    observeEvent(input$hcClicked, {
        showModal(modalDialog(
            tagList(renderHighchart({
                x <-
                    state_daily()[state_daily()$Country.Region == input$hcClicked, c("Country.Region",
                                                                                     "Date",
                                                                                     input$analysis_type_india)]
                colnames(x) <-
                    c("Country.Region", "Date", "Value")
                type <- "linear"
                if (input$analysis_type_india %in% c("Confirmed(#)", "New Confirmed(#)")) {
                    color <- "#f39c12"
                }
                else if (input$analysis_type_india %in% c(
                    "Deaths(#)",
                    "Overall Death Rate(%)",
                    "Finalised Death Rate(%)",
                    "New Deaths(#)"
                    ...
                    color <- "#3c8dbc"
                if (input$analysis_type_india %in% c(
                    "Confirmed(#)",
                    "New Confirmed(#)",
                    "Deaths(#)",
                    "New Deaths(#)",
                    ...
                    type <- ifelse(input$axis_type3 == "Log10",
                                   "logarithmic",
                                   "linear")
                }
                p <-
                    linechart(x, input$analysis_type_india, type)
                if (input$analysis_type_india %in% c(
                    "Growth Factor",
                    "Overall Death Rate(%)",
                    "Finalised Death Rate(%)",
                    ...
                    p <- p %>% hc_tooltip(valueDecimals = 2, shared = TRUE)
                }
            ...
            renderHighchart({
                district_data <- district_data()
                data <-
                    district_data[district_data$State == input$hcClicked, ]
                ...
                if (var > 0) {
                    hctreemap2(
                        data,
                        group_vars = c("District"),
                        size_var = "Confirmed",
                        color_var = "New_Confirmed",
                        layoutAlgorithm = "squarified"
                    ) %>%
                        hc_tooltip(
                            pointFormat = "<b>{point.name}</b>:<br>
            Confirmed: {point.value:,.0f}<br> New Confirmed: {point.colorValue:,.0f}"
                        ) %>% hc_legend(enabled = F) %>%
                        hc_colorAxis(minColor = brewer.pal(9, "Reds")[3],
                                     maxColor = brewer.pal(9, "Reds")[9]) %>% hc_exporting(enabled = TRUE)
                } else{
                    hctreemap2(
                        data,
                        group_vars = c("District"),
    ...
    output$plot_statewise <- renderHighchart({
        myClickFunc <-
            JS("function(event) {Shiny.onInputChange('hcClicked', event.point.name);}")
        x <-
            india_statewise()[india_statewise()$Country.Region != "India", c("Country.Region", "Date", input$analysis_type_india)]
        colnames(x) <- c("Country.Region", "Date", "Value")
        colorByPoint <- FALSE
        type <- "linear"
        if (input$analysis_type_india %in% c("Confirmed(#)", "New Confirmed(#)")) {
        ...
        p <-
            hchart(x,
                   "bar",
                   hcaes(x = Country.Region, y = Value),
                   name = input$analysis_type_india) %>%
            hc_yAxis(title = list(text = input$analysis_type_india),
                     type = type) %>% hc_xAxis(title = list(text = "State/UT"))
        if (input$analysis_type_india %in% c(
            "Growth Factor",
            "Overall Death Rate(%)",
            "Finalised Death Rate(%)",
        ...
        p %>% hc_title(text = "<b>Latest State/Union Territory Split</b>", align = 'left') %>%
            hc_plotOptions(
                bar = list(colorByPoint = colorByPoint),
                series = list(
                    stacking = FALSE,
    ...
    output$testing_datatable <- DT::renderDataTable({
        ...
        x$updatetimestamp <-
            as.POSIXct(x$updatetimestamp, format = '%d/%m/%Y %H:%M:%S')
        x <- x[order(x$updatetimestamp, decreasing = T), ]
        x$updatetimestamp <-
            format(x$updatetimestamp, format = "%d %B %Y, %I:%M:%S %p IST")
        x[colnames(x)[2:(length(colnames(x)) - 1)]] <-
            apply(x[colnames(x)[2:(length(colnames(x)) - 1)]], 2, as.numeric)
        x$pctpostivecases <-
            x$totalpositivecases / x$totalsamplestested * 100
        
        n <- colnames(x)[1:(length(colnames(x)) - 2)]
        x <- x[c(n, "pctpostivecases", "updatetimestamp")]
        DT::datatable(x, rownames = FALSE, filter = "top") %>%
            formatCurrency(
                colnames(x)[2:(length(colnames(x)) - 2)],
                currency = "",
                interval = 3,
                mark = ",",
                digits = 0
    ...
    output$value_boxes3 <- renderUI({
        x <- india_statewise()
        cnf <-
            comma(x[x$Country.Region == "India", "Confirmed(#)"], digits = 0)
        cnf_new <-
            comma(x[x$Country.Region == "India", "New Confirmed(#)"], digits = 0)
        dth <-
            comma(x[x$Country.Region == "India", "Deaths(#)"], digits = 0)
        dth_new <-
            comma(x[x$Country.Region == "India", "New Deaths(#)"], digits = 0)
        rec <-
            comma(x[x$Country.Region == "India", "Recovered(#)"], digits = 0)
        rec_new <-
            comma(x[x$Country.Region == "India", "New Recovered(#)"], digits = 0)
        act <-
            comma(x[x$Country.Region == "India", "Active(#)"], digits = 0)
        list(
            valueBox(
                cnf,
                icon = icon("plus-square"),
    ...
    output$updt_time <- renderText({
        ...
        paste(
            "<B>Last Updated:</B>",
            format(as.POSIXct(y, format = '%d/%m/%Y %H:%M:%S'), format = "%d %B %Y, %I:%M:%S %p IST")
        )
    })
    ...
    data_table_india <- reactive({
        ...
        y <- a()$statewise
        y$lastupdatedtime <-
            format(as.POSIXct(y$lastupdatedtime, format = '%d/%m/%Y %H:%M:%S'),
    ...
    fcst_data <- reactive({
        x <- india()
        reg_data <-
            x[c("Date", "Confirmed(#)", "Deaths(#)", "Recovered(#)")]
        ...
        pred.type <-
            sapply(c("Confirmed(#)", "Deaths(#)", "Recovered(#)"), function(x) {
                if (x == "Recovered(#)") {
                    if (input$scenario %in% c("Very Optimistic", "Mildly Optimistic")) {
                        "upr"
                    }
                    else if (input$scenario %in% c("Severe", "Extremely Severe")) {
                        "lwr"
                    }
                    else {
                        "fit"
        ...
        final_data <-
            sapply(c("Confirmed(#)", "Deaths(#)", "Recovered(#)"), function(x) {
                a <- do.call('rbind', sapply(c(3, 7, 14, nrow(
                    reg_data
                )), function(l) {
                    ...
                    p <-
                        as.data.frame(predict.lm(
                            reg,
                            data.frame(x = c(l, l + 3, l + 7, l + 14, l + 30)),
                            level = level,
                            interval = "prediction"
                        ))
        ...
        b <-
            final_data[["Confirmed(#)"]] - final_data[["Deaths(#)"]]
        final_data[["Deaths(#)"]][b < 0] <-
            final_data[["Confirmed(#)"]][b < 0]
        b <-
            final_data[["Confirmed(#)"]] - final_data[["Recovered(#)"]]
        final_data[["Recovered(#)"]][b < 0] <-
            final_data[["Confirmed(#)"]][b < 0]
        
        final_data[["Active(#)"]] <-
            final_data[["Confirmed(#)"]] - final_data[["Deaths(#)"]] - final_data[["Recovered(#)"]]
        final_data[["Active(#)"]][final_data[["Active(#)"]] < 0] <-
            0
        final_data[["Active Rate(%)"]] <-
            final_data[["Active(#)"]] / final_data[["Confirmed(#)"]] * 100
        final_data[["Overall Death Rate(%)"]] <-
            final_data[["Deaths(#)"]] / final_data[["Confirmed(#)"]] * 100
        final_data[["Finalised Death Rate(%)"]] <-
            final_data[["Deaths(#)"]] / (final_data[["Deaths(#)"]] + final_data[["Recovered(#)"]]) *
            100
        final_data[["Overall Recovery Rate(%)"]] <-
            final_data[["Recovered(#)"]] / final_data[["Confirmed(#)"]] * 100
        final_data[["Finalised Recovery Rate(%)"]] <-
            final_data[["Recovered(#)"]] / (final_data[["Deaths(#)"]] + final_data[["Recovered(#)"]]) *
            100
    ...
    output$fcst_data_table <- DT::renderDataTable({
        ...
        if (input$analysis_type_india %in% c("Confirmed(#)",
                                             "Deaths(#)",
                                             "Recovered(#)",
                                             "Active(#)")) {
            DT::datatable(
    ...
    output$fcst_plot <- renderHighchart({
        ...
        d$Growth_Period_Estimated_From <-
            paste("Growth -", d$Growth_Period_Estimated_From)
        type <- "linear"
        type <-
            ifelse(input$axis_type_fcst == "Linear",
                   "linear",
                   "logarithmic")
        highcharter::hchart(d,
                            type = 'column',
                            hcaes(
                                y = value,
                                group = Growth_Period_Estimated_From,
                                x = variable
                            ))  %>%
            hc_yAxis(title = list(text = input$analysis_type_india),
                     type = type) %>% hc_xAxis(title = list(text = "")) %>%
    ...
    output$fcst_data_table_world <- DT::renderDataTable({
        ...
        pred.type <-
            sapply(c("Confirmed(#)", "Deaths(#)", "Recovered(#)"), function(x) {
                if (x == "Recovered(#)") {
                    if (input$scenario2 %in% c("Very Optimistic", "Mildly Optimistic")) {
                        "upr"
                    }
                    else if (input$scenario2 %in% c("Severe", "Extremely Severe")) {
                        "lwr"
                    }
                    else {
                        "fit"
        ...
        if (is.null(d)) {
            return("Please choose other analysis type")
        }
        if (input$analysis_type_world %in% c("Confirmed(#)",
                                             "Deaths(#)",
                                             "Recovered(#)",
                                             "Active(#)")) {
            d <- DT::datatable(
    ...
    output$fcst_plot_world <- renderHighchart({
        ...
        type <- "linear"
        type <-
            ifelse(input$axis_type_fcst_world == "Linear",
                   "linear",
                   "logarithmic")
        highcharter::hchart(d,
                            type = 'column',
            ...
            hc_yAxis(title = list(text = input$analysis_type_world),
                     type = type) %>% hc_xAxis(title = list(text = "")) %>%
            hc_tooltip(valueDecimals = 0, shared = TRUE) %>%
            hc_chart(zoomType = 'xy') %>% hc_exporting(enabled = TRUE)
    ...
    output$news_india <- DT::renderDataTable({
        ...
        df$ago <-
            sapply(df$published_at, time_diff, current_time = time())
        df$url2 <-
            paste0("<a href='",
                   df$url,
                   "' target='_blank'>",
                   ...
                   "</a>")
        df$image <-
            paste0('<img src="', df$url_to_image, '" height="80"></img>')
        df$url2 <-
            paste(
                df$url2,
                df$description,
    ...
    output$news_world <- DT::renderDataTable({
        df_country <-
            get_everything(
                query = paste("COVID-19", input$news_country),
                from = Sys.Date() - 1,
                to = Sys.Date(),
                sort_by = "relevancy",
                language = "en"
    ...
    output$plot_modal_sate <- renderHighchart({
        ...
        x <-
            state_daily()[state_daily()$Country.Region %in% c , c("Country.Region", "Date", input$analysis_type_state)]
        colnames(x) <- c("Country.Region", "Date", "Value")
        type <- "linear"
        if (input$analysis_type_state %in% c(
            "Confirmed(#)",
            "New Confirmed(#)",
            "Deaths(#)",
            "New Deaths(#)",
            ...
            type <- ifelse(input$axis_type5 == "Log10",
                           "logarithmic",
                           "linear")
        }
        ...
        bsModal(
            ...
            column(
                ...
                selectInput(
                    "analysis_type_state",
                    label = "Select",
                    choices = names(state_daily()[3:15])
                )
            ),
    ...
    output$source_heading <- renderUI({
        a <- h1(strong("About the Developer"))
        b <-
            h4(p(
                strong("Abhijatri Das"),
                ", Credit Risk Modeller, Statistician and a Data Enthusiast"
            ))
        email <-
            h4(
                icon("envelope"),
                shiny::a(
                    "abhijatridas@gmail.com",
                    href = "mailto:abhijatridas@gmail.com",
                    target = "_blank"
                )
            )
        link <-
            h4(
                icon("linkedin") ,
                shiny::a(
                    "https://www.linkedin.com/in/abhijatri-das-2800454b",
                    href = "https://www.linkedin.com/in/abhijatri-das-2800454b",
                    target = "_blank"
                )
            )
        c <- h1(strong("Data Source"))
        url1 <-
            shiny::a(
                "India COVID-19 Tracker - A Crowdsourced Initiative",
                icon("external-link"),
                href = "https://api.covid19india.org",
                target = "_blank"
            )
        ...
        d <- h4(strong("India Live Data: "), url1)
        e <- h4(strong("World Daily Data: "), url2)
        f <-
            p(
    ...
    output$value_boxes4 <- renderUI({
        ...
        cnf <-
            comma(x[x$Country.Region == "Australia", "Confirmed(#)"], digits = 0)
        cnf_new <-
            comma(x[x$Country.Region == "Australia", "New Confirmed(#)"], digits = 0)
        dth <-
            comma(x[x$Country.Region == "Australia", "Deaths(#)"], digits = 0)
        dth_new <-
            comma(x[x$Country.Region == "Australia", "New Deaths(#)"], digits = 0)
        rec <-
            comma(x[x$Country.Region == "Australia", "Recovered(#)"], digits = 0)
        rec_new <-
            comma(x[x$Country.Region == "Australia", "New Recovered(#)"], digits = 0)
        act <-
            comma(x[x$Country.Region == "Australia", "Active(#)"], digits = 0)
        list(
            valueBox(
                cnf,
                icon = icon("plus-square"),
    ...
    output$analysis_type_australia_select_input <- renderUI({
        selectInput(
            "analysis_type_australia",
            label = "Select",
            choices = names(australia)[3:15]
        )
    })
    #
    ...
    output$plot_australia <- renderHighchart({
        x <-
            australia[australia$Country.Region == "Australia", c("Country.Region",
                                                                 "Date",
                                                                 input$analysis_type_australia)]
        colnames(x) <- c("Country.Region", "Date", "Value")
        type <- "linear"
        if (input$analysis_type_australia %in% c("Confirmed(#)", "New Confirmed(#)")) {
            color <- "#f39c12"
        } else if (input$analysis_type_australia %in% c(
            "Deaths(#)",
            "Overall Death Rate(%)",
            "Finalised Death Rate(%)",
            "New Deaths(#)"
        ...
        if (input$analysis_type_australia %in% c(
            "Confirmed(#)",
            "New Confirmed(#)",
            "Deaths(#)",
            "New Deaths(#)",
            ...
            type <-
                ifelse(input$axis_type_australia == "Log10",
                       "logarithmic",
                       "linear")
        }
    ...
    output$plot_statewise_australia <- renderHighchart({
        myClickFunc <-
            JS(
                "function(event) {Shiny.onInputChange('hcClicked_aus', event.point.name);}"
            )
        x <-
            australia[australia$Date == input$date_input_aus &
                          australia$Country.Region != "Australia", c("Country.Region", input$analysis_type_australia)]
        colnames(x)[2] <- "Value"
        type <- "linear"
        ...
        p <- hcmap(
            ...
            value = "Value",
            name = input$analysis_type_australia,
            dataLabels = list(enabled = TRUE, format = '{point.name}'),
            download_map_data = F
        ) %>%
            hc_colorAxis(
    ...
    observeEvent(input$hcClicked_aus, {
        showModal(modalDialog(renderHighchart({
            x <-
                australia[australia$Country.Region == input$hcClicked_aus, c("Country.Region",
                                                                             "Date",
                                                                             input$analysis_type_australia)]
            colnames(x) <-
                c("Country.Region", "Date", "Value")
            ...
            p <-
                linechart(x, input$analysis_type_australia, type)
            if (input$analysis_type_australia %in% c(
                "Growth Factor",
                "Overall Death Rate(%)",
                "Finalised Death Rate(%)",
    ...
    output$plot_statewise_india <- renderHighchart({
        a <- state_daily()
        a <-
            a[a$Date == input$date_input_ind &
                  a$Country.Region != "India", c("Country.Region", input$analysis_type_india)]
        colnames(a)[2] <- "Value"
        a <- a[order(a$Country.Region), ]
        a[which(!a$Country.Region %in% c("Delhi", "Ladakh")), "Country.Region"] <-
            chk2
        a[a$Country.Region == "Delhi", "Country.Region"] <-
            "NCT of Delhi"
        a[a$Country.Region == "Ladakh", "Country.Region"] <-
            "Jammu and Kashmir"
        a <-
            a %>% group_by(Country.Region) %>% summarise(Value = sum(Value, na.rm = F))
        type <- "linear"
    ...
    output$fcst_data_table_australia <- DT::renderDataTable({
        ...
        pred.type <-
            sapply(c("Confirmed(#)", "Deaths(#)", "Recovered(#)"), function(x) {
                if (x == "Recovered(#)") {
                    if (input$scenario3 %in% c("Very Optimistic", "Mildly Optimistic")) {
                        "upr"
                    }
                    else if (input$scenario3 %in% c("Severe", "Extremely Severe")) {
                        "lwr"
                    }
                    else {
                        "fit"
        ...
        if (input$analysis_type_australia %in% c("Confirmed(#)",
                                                 "Deaths(#)",
                                                 "Recovered(#)",
                                                 "Active(#)")) {
            d <- DT::datatable(
    ...
    output$fcst_plot_australia <- renderHighchart({
        ...
        type <- "linear"
        type <-
            ifelse(input$axis_type_fcst_australia == "Linear",
                   "linear",
                   "logarithmic")
        highcharter::hchart(d,
                            type = 'column',
            ...
            hc_yAxis(title = list(text = input$analysis_type_australia),
                     type = type) %>% hc_xAxis(title = list(text = "")) %>%
            hc_tooltip(valueDecimals = 0, shared = TRUE) %>%
            hc_chart(zoomType = 'xy') %>% hc_exporting(enabled = TRUE)
    ...
    output$plot_modal_australia <- renderHighchart({
        c <-
            data_table_australia()[input$data_table_australia_rows_selected, 1]
        x <-
            australia[australia$Country.Region %in% c , c("Country.Region",
                                                          "Date",
                                                          input$analysis_type_australia_modal)]
        colnames(x) <- c("Country.Region", "Date", "Value")
        type <- "linear"
        if (input$analysis_type_australia_modal %in% c(
            "Confirmed(#)",
            "New Confirmed(#)",
            "Deaths(#)",
            "New Deaths(#)",
            ...
            type <-
                ifelse(input$axis_type_australia_modal == "Log10",
                       "logarithmic",
                       "linear")
        }
    ...
    observeEvent(input$sir_bttn, {
        ...
        x <- all[all$Country == country & all$`Confirmed(#)` >= 1 &
                     all$Date >= input$daterange2[1] &
                     all$Date <= input$daterange2[2], c("Date",
                                                        "Confirmed(#)",
                                                        "Active(#)",
                                                        "Deaths(#)",
                                                        "Recovered(#)")]
        ...
        x <-
            data.frame(S = N - x['Confirmed(#)'],
                       I = x["Active(#)"],
                       R = x["Deaths(#)"] + x["Recovered(#)"])
        colnames(x) <- c("S", "I", "R")
        ...
        model.overtime <-
            1:(length(Day) + input$nahead) # time in days
        
        fit <-
            data.frame(ode(
                y = initialvalues,
                times = model.overtime,
                func = SIR,
                parms = opt.par
            ))
        ...
        output$plot_sir <- renderHighchart({
            type <-
                ifelse(input$axis_type_sir == "Linear",
                       "linear",
                       "logarithmic")
            highcharter::hchart(
                x,
        ...
        output$sir_result <- renderUI({
            beta <-
                h5(strong("Estimated Infection Rate (Beta) = "),
                   round(opt.par['beta'] * 100, 2),
                   "%")
            gamma <-
                h5(strong("Estimated Recovery Rate (Gamma) = "),
                   round(opt.par['gamma'] * 100, 2),
                   "%")
            list(beta, gamma)
        })
    })
    ...
    observe({
        ...
        # Set the SIR Beta and Gamma parameters
        opt.par <-
            setNames(as.numeric(c(input$beta2, input$gamma2)), c("beta", "gamma"))
        ...
        colnames(fit) <-
            c("Date", "Susceptible", "Infected", "Removed")
        ...
        output$plot_toy_sir <- renderHighchart({
            ...
            highcharter::hchart(
                ...
                hc_yAxis(
                    title = list(text = "% of Susceptible"),
                    type = type,
                    min = 0,
                    max = 100
                ) %>%
    ...
    output$bubble_plot <- renderHighchart({
        ...
        highchart() %>%
            hc_add_series_scatter(
                ...
                z = x$Population,
                label = x$iso3,
                color = x$Country.Region,
                dataLabels = list(enabled = TRUE)
            ) %>%
            hc_legend(enabled = FALSE) %>%
            hc_xAxis(
                type = type,
                title = list(text = "Cases per million"),
                crosshair = TRUE
            ) %>%
            hc_yAxis(
                type = type,
                title = list(text = "Deaths per 100 cases"),
                crosshair = TRUE
            ) %>%
            hc_zAxis(title = list(text = "Ratio")) %>%
            hc_colors(colors = gg_color_hue(length(
                unique(cnf_dth_pop$Country.Region)
            ))) %>%
            hc_tooltip(
                headerFormat = '',
            ...
            hc_subtitle(text = format(date, format = "%d %B %Y"),
                        align = 'left')
    })
}
